...
def najmanji_kvadrat_lite(file,colour="red",xlabel="x os", ylabel="y os", title="x/y graf",linelegend="graf linearne regresije",scatterlegend="podaci"):
    with open (file, "r") as csv_file:
        ...
        print("pogreška a=",sigma_a)
        print("n=",n_mean)
        sigma_n=np.sqrt(np.sum([(i-n_mean)**2 for i in n])/((len(n)-1)*len(n)))
        print("pogreška n=",sigma_n)
        print("R=",(koeficijent_smjera*(101400)/(n_mean)))
        print("pogreška R=",(sigma_n*koeficijent_smjera*(101400)/((n_mean**2))))
def stand_dev(lista):
    sum=0
    for i in lista: 
